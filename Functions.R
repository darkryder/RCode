...
getSizeFactor <- function(geneCount, group){
  ...
  ## New normalisation
  sorted <- apply(geneCount, 2, sort, TRUE)
  NormedGene <- apply(sorted, 2, function(x,c){x / c}, sorted[,1])
  Deb <- apply(NormedGene, 2, medianWithoutNA)
  return(list(TC=TC,UQ=UQ,Med=Med,DESeq=deseq,Deb=Deb,TMM=f.scale))
}
